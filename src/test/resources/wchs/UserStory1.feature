...
Feature: User Story 1
  ...
  Scenario: Application allows the creation of a single working class hero via API call. (Positive flow)
  ...
  8. browniePoints and deathDate are nullable
    ...
    Given I delete working class heros record with natid '<natid>'
  ...
  @US1_AC2 @TEST1
  Scenario Outline: Application allows the creation of a single working class hero via API call.
  AC2: Field validations are as follow
  1. natid must be in the format “natid-number”, where number is between 0 to 9999999
  (inclusive)
  2. name must contain letters and spaces with a length between 1 and 100 (inclusive)
